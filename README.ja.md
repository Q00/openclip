...
**2. `oc` スキルを呼び出し、やりたいことを伝えます**：

```
you    $oc ./talk.mp4 からショート動画を作って
...
お使いのエージェントを開き、*`$oc この動画からショート動画を作って`* と呼び出してください。自然言語だけでも動作しますが、最初の実行では明示的なスキル呼び出しが最も確実です。スキルフォルダはフローマニフェストと
...
実行可能なすべてのショートと長尺の候補を、日本語字幕付きで生成する：
